...
def get_closest_color(target_color, available_colors):
    ...
    hsv_available_colors = []
    for i in range(len(available_colors)):
        hsv_available_colors.append(rgb_to_hsv(available_colors[i]))
    hsv_available_colors = np.array(hsv_available_colors)
    target_color = np.array(target_color)
    target_color = rgb_to_hsv(target_color)
    # print(target_color)
    # print(hsv_available_colors)

    distances = np.sqrt(np.sum((hsv_available_colors-target_color)**2,axis=1))
...
if __name__ == "__main__":
    ...
    # color_names = ["grass", 'red', 'blue', 'green', 'black', 'white', 'yellow', 'orange', 'pink', 'purple', 'brown', 'grey', 'turquoise', 'light green', 'light blue', 'dark green', 'pastel blue', 'pastel green', 'pastel yellow', 'pastel pink', 'pastel purple', 'pastel coral', 'fuchsia', 'raspberry', 'burgundy', 'red brown', 'light brown', 'beige', 'teddy bear', 'cream', 'flesh']
